...
EXP_ST u32* pointer_bits;                /* SHM with instrumentation bitmap  */
EXP_ST u8* memseq_bits;                /* SHM with instrumentation bitmap  */

EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           virgin_tmout[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
           virgin_crash[MAP_SIZE];    /* Bits we haven't seen in crashes  */
           virgin_memseq_bits[MAP_MEMSEQ_SIZE];    /* Bits we haven't seen in crashes  */ 
...
static u32 pointer_score = 0;             //global pointer score

struct queue_entry {
	
  u32 queue_id;                       // id the entry
  u32 pointer_score;                  //In the current seed' u32 SHM[MAP_SIZE], use to sort the queue high->lower
...
/* Add for guide: to get the pointer score of a seed */
u32 get_pointer_score(u32* pointer_score_map) {

  u32 ret = 0;
  if(pointer_score_map[0] > 0)
  {
      ret = pointer_score_map[0];
  }
  //printf("pointer_score %d \n", ret);
  return ret;
}
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  q->pointer_score = pointer_score; //get at pointer map
...
/* Check if the current execution memseq brings anything new to the table.*/

static inline u8 has_new_memseq_bits(u8* virgin_map) {

#ifdef __x86_64__

  u64* current = (u64*)memseq_bits;
    ...
    if (unlikely(*current) && unlikely(*current & *virgin)) {
      ...
      if (likely(ret < 2)) {
        ...
        if ((cur[0] && vir[0] == 0xff) || (cur[1] && vir[1] == 0xff) ||
			...
			{
				ret = 2;
				//new_memseq_count=new_memseq_count + 2;
			}
        else 
			{
				ret = 1;
				//new_memseq_count=new_memseq_count + 1;
			}
      ...
      //a = a + ((vir[0]^ 0xff)*cur[0] + (vir[1]^ 0xff)*cur[1] + (vir[2]^ 0xff)*cur[2] + (vir[3]^ 0xff)*cur[3] 
      //+ (vir[4]^ 0xff)*cur[4] + (vir[5]^ 0xff)*cur[5] + (vir[6]^ 0xff)*cur[6] + (vir[7]^ 0xff)*cur[7]);

      //b = b + ( (vir[0]^0xff)*(vir[0]^0xff)  + (vir[1]^0xff)*(vir[1]^0xff) +(vir[2]^0xff)*(vir[2]^0xff) + (vir[3]^0xff)*(vir[3]^0xff)
      // + (vir[4]^0xff)*(vir[4]^0xff)+ (vir[5]^0xff)*(vir[5]^0xff)+ (vir[6]^0xff)*(vir[6]^0xff)+ (vir[7]^0xff)*(vir[7]^0xff) );

      //c = c + ( cur[0]*cur[0] + cur[1]*cur[1] + cur[2]*cur[2] + cur[3]*cur[3] + cur[4]*cur[4] + cur[5]*cur[5] + cur[6]*cur[6] + cur[7]*cur[7] );
  ...
  //if(a>0 && b>0 && c>0)
  //{
    //memseq_distance = a / (sqrt(b) * sqrt(c));
  //}
  //else
  //{
    //memseq_distance = 0;
  ...
  //if (ret && virgin_map == virgin_pointer_bits) bitmap_changed = 1;
...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  u64 pointer_factor = q->pointer_score;
  ...
  for (i = 0; i < MAP_SIZE; i++)
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
         ...
         // Higher pointer test cases are favored. pointer_score favor prior than (Faster-executing mul smaller test cases)

         if (pointer_factor <= top_rated[i]->pointer_score) 
         { 
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) memset(virgin_memseq_bits, 255, MAP_MEMSEQ_SIZE);  

  memset(virgin_tmout, 255, MAP_SIZE);
  memset(virgin_crash, 255, MAP_SIZE);

  shm_id = shmget(IPC_PRIVATE, MAP_SIZE + MAP_POINTER_SIZE*sizeof(u32) + MAP_MEMSEQ_SIZE, IPC_CREAT | IPC_EXCL | 0600);
  ...
  pointer_bits = (u32 *)(trace_bits + MAP_SIZE); // pointer score map
  memseq_bits = (u8 *)(trace_bits + MAP_SIZE + MAP_POINTER_SIZE * sizeof(u32)); // memseq score map
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  memset(trace_bits, 0, MAP_SIZE + MAP_POINTER_SIZE * sizeof(u32)); 
  MEM_BARRIER();
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    /* Keep only if there are new bits in the map, add to queue for
       future fuzzing, etc. */
    hns=has_new_memseq_bits(virgin_memseq_bits);	
...
#ifndef SIMPLE_FILES
    pointer_score = get_pointer_score(pointer_bits); //get pointer_bits[0] as score

    fn = alloc_printf("%s/queue/id:%06u,%s,%d", out_dir, queued_paths,
                      describe_op(hnb), pointer_score);
